...
export default function Navbar() {
    ...
    return <header className="flex flex-row relative h-[110px] w-[100vw] bg-[#CFB290] shadow-md border-b-2">
        ...
        <div className="hidden md:flex md:flex-row w-[33vw] justify-center items-center">
               <button className="w-[22%] border border-[#593E2B] bg-[#593E2B] px-4 py-3 text-[1rem] tracking-wide text-[#CFB290] font-semibold uppercase rounded-lg  hover:bg-[#3d2a1d] transition-all duration-200 cursor-pointer mx-4"> <Link to='/register'>Sign Up</Link></button>
        ...
        <div className="absolute top-7 cursor-pointer hover:opacity-70 right-4 md:hidden z-10" onClick={()=>{setMenuOpen(!ismenuOpen)}}>
                <img src={ismenuOpen ? "icons/close.png" : 'icons/menu.png'} className="w-10" alt="" />
                
        </div>
        <div className={`md:hidden w-[100vw] h-[100vh] transition-all duration-300 ease-in-out bg-[#CFB290] bg-opacity-70 flex flex-col items-center absolute left-0 top-0 pt-10 gap-7 justify-center z-0 ${ismenuOpen ? 'translate-x-0 ': 'translate-x-[100vw]'} `}>
